...
def join_components(
    ...
    :return:
    """
    components = (sparsified_graph.subgraph(c).copy() for c in connected_components(sparsified_graph))
...
def remove_small_components(graph: Graph, minsize):
    ...
    components = (graph.subgraph(c).copy() for c in connected_components(graph))
