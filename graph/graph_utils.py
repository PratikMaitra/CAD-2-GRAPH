...
def plot_dis(dis_dic, name, weight = "weight"):
    
    fig, ax = plt.subplots( nrows=1, ncols=1 )  # create figure & 1 axis
    plt.scatter(*zip(*dis_dic.values()))
    corr, _  = pearsonr(*zip(*dis_dic.values()))
    print("r =",corr)
    ...
    def abline(slope, intercept):
        ...
        text_idx = int(len(x_vals)/2)
        
        plt.text(x_vals[text_idx], y_vals[text_idx], slope, horizontalalignment='center',
    ...
    fig.savefig("dis_scatter "+weight+" "+name+".png")   # save the figure to file
