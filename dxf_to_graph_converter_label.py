...
def extract_graph_from_dxf(architecture_filename, label_filename, outfile, building_name, step_size=None):
    ...
    if os.path.exists(f"{outfile}.yaml"):
        os.remove(f"{outfile}.yaml")

    graph_4d = make_4d_nodes(large_components_graph, floor=outfile.split("/")[-1], building=building_name)
    with open(f"{outfile}.yaml", "w") as f:
        yaml.dump((graph_4d, f"{outfile}.yaml"), f)
    logging.info(f"{outfile} created")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-fa', '--architecture_filename', type=str, default="cad_files/BoydTower/Architecture/BTAR02.dxf", help="path to input architecture file")
    parser.add_argument('-fl', '--label_filename', type=str, default="cad_files/BoydTower/Space/BTSP02.dxf", help="path to input label file")
    parser.add_argument('-o', '--outfile', type=str, default="out_test" ,help="path to output file")
    ...
    parser.add_argument('-bl', '--building_name', type=str, default="Unknown_Bld_Name", help="The name of the building for this CAD file")
    parser.add_argument('-l', '--labels', action="store_true", help="Labels toggle option")
    ...
    #     from graph_to_svg.svg_saver import export_graph_overlay_on_cad
    # else:
    #     from graph_to_svg.svg_saver_off import export_graph_overlay_on_cad
