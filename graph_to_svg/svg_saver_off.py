...
def export_graph_overlay_on_cad(dxf: DXF, graph, grid_size, canvas_lims, outfile, label=False):
    with open(outfile, "w+") as dwg:
        ...
        for node in graph.nodes:
            if graph.nodes[node]["type"] == SpaceType.OPEN:
                dwg.write(draw_circle(node.x, node.y, grid_size, "rgb(0,0,0)", label))
            elif graph.nodes[node]["type"] == SpaceType.DOOR:
                dwg.write(draw_circle(node.x, node.y, grid_size, "rgb(255,0,0)", label))
            else:
                dwg.write(draw_circle(node.x, node.y, grid_size, "rgb(0,0,0)", label))
            if label:
        ...
        for edge in graph.edges:
            if "type" not in graph.edges[edge]:
                dwg.write(draw_edge(edge, grid_size, color="rgb(153,0,0)", bold=label))
            elif graph.edges[edge]["type"] == "door":
                dwg.write(draw_edge(edge, grid_size, color="rgb(153,76,0)", bold=label))
            else:
                dwg.write(draw_edge(edge, grid_size, bold=label))
